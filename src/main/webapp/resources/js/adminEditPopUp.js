...
// -1 == 초기값 ,  0 == 컨텐츠 삭제(x버튼),  1 == 새 이미지 업로드
...
if(imageInput != null){ // 화면에 imageInput이 있을 경우 ( if 굳이 안해도 되긴 함 ) 
    ...
    imageInput.addEventListener("change", e => {
        ...
        if(file == undefined){ 
            ...
            // 취소 시 기존 컨텐츠 이미지로 변경 ( 기존 이미지에서 변경되는게 없게 하겠다는거죠 ) 
    ...
    deleteImage.addEventListener('click', () => {
        ...
        uploadPlaceImg.setAttribute("src", "/resources/img/common/main/프로필아이콘.png");
        // 컨텐츠 이미지를 기본 이미지로 변경
    ...
    document.getElementById("profileFrm").addEventListener("submit", e => {
        ...
        // 이전 컨텐츠 이미지가 없으면서, 새 이미지 업로드를 했다 -> 처음으로 이미지 추가
        if(!initCheck && deleteCheck == 1)  flag = false;

        // 이전 컨텐츠 이미지가 있으면서, 새 이미지 업로드를 했다 -> 새 이미지로 변경
        if(initCheck && deleteCheck == 1)   flag = false;
        
        // 이전 컨텐츠 이미지가 있으면서, 컨텐츠 삭제 버튼을 눌렀다 -> 삭제
